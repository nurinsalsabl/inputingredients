...
def load_queue_data():
    try:
        ...
        df = pd.read_csv("products_ingredients_under_13_queue.csv")
        ...
        st.error(f"Gagal membaca file 'products_ingredients_under_13_queue.csv': {e}")
        return pd.DataFrame()

df_queue = load_queue_data()

if df_queue.empty:
    st.warning("File queue produk kosong atau tidak ditemukan. Pastikan 'products_ingredients_under_13_queue.csv' berada di folder yang sama.")
    st.stop()
